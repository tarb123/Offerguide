...
export async function POST(request: Request) {
  try {
    ...
    if (action === "signup") {
      ...
      const existingUser = await CandidateUser.findOne({
      ...
      const user = await CandidateUser.create({
        fullName,
        email: email.toLowerCase(),
    ...
    if (action === "login") {
      ...
      if (!jwtSecret) {
        console.error("Candidate Auth API Error: JWT_SECRET is not set.");
        return NextResponse.json(
  ...
  return response;
    ...
    if (action === "forgot-password") {
      ...
      await transporter.sendMail({
        ...
        to: user.email,
        subject: "Sanjeeda.io Candidate Password Reset OTP",
    ...
    console.error("Candidate Auth API Error:", error);
