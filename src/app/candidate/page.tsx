...
async function callApi(action: string, payload: object) {
  ...
  try {
    const response = await fetch("/api/pgp-candidate/auth", {
      method: "POST",
      headers: {
        ...
        <section className="flex items-center justify-center bg-slate-50 dark:bg-transparent px-5 py-10 sm:px-10">
          <div className="w-full max-w-md">
           ...
           <div className="mb-3 flex bg-white dark:bg-transparent p-1.5">
              ...
              <AuthTab label="Sign In = Candidate" active={mode === "login"} onClick={() => setMode("login")} />
              <AuthTab label="Sign Up = Candidate" active={mode === "signup"} onClick={() => setMode("signup")} />
            </div>
